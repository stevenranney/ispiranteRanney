...
`ForecasthelpDeskCalls()` will be able to predict out as far or as short as the user would like.  Indeed, narrow temporal windows would likely provide the most use.  However, monthly, quarterly, or even yearly predictions are possible.  Regardless, like any statistical model, models are only as good as the data they are based on.
...
These functions would never need to be accessed by the user.  They are called from within the higher level functions on the principle of "don't repeat yourself."
