...
PlotResolutionTimeByDay <- structure(
function 
##title<< Plot hours to resolution by day of week 
  ...
  ## hours) and produces a box plot of those values as a function of \code{day}.
  ## Day levels include those assigned from the \code{\link{AssignDateAndDay}} 
  ## function.  
  #
  ...
  if(length(which(is.na(dF$value))) == nrow(dF)){
    stop(print("There is no resolution time data in this data frame."))
    } else {
  
  ##details<< Resolution time in all sample dataFrames is a \code{factor}; this function
